...
from backend.core.database import RunHistoryStore
...
class HallucinationPipeline:
    def __init__(
        ...
        # All three are overridable for tests (avoids hitting the real
        # Gemini API, downloading the NLI model, or touching disk in unit
        # tests); production code builds them from env vars via the same
        # factories/wiring the API layer uses. db_path=None defaults to an
        # isolated in-memory SQLite db per pipeline instance -- matching the
        # old deque's per-instance, ephemeral behavior for tests -- while
        # app.py explicitly passes a real file path for persistence.
        self._generator = generator or AnswerGenerator()
        self._grounding_scorer = grounding_scorer or build_default_grounding_scorer()
        self._run_store = RunHistoryStore(db_path)
    ...
    def recent_runs(self, limit: int = 10) -> RunHistoryResponse:
        runs = self._run_store.recent(limit)
        return RunHistoryResponse(total_runs=self._run_store.total_count(), runs=runs)

    def analytics_summary(self) -> AnalyticsSummaryResponse:
        return self._run_store.summary()
    ...
    def _execute(
        ...
        *,
        ...
        """Shared core of run_query/run_detection: score, repair, record, respond.

        Both public methods only differ in how `answer` and the
        generation_* metadata are obtained (LLM/template generation vs.
        user-supplied text) -- everything after that point (claim scoring,
        the repair pass, confidence/hallucination-score math, run history,
        response assembly) was previously duplicated between them and is
        now written once.
        """
        ...
        corrected_answer = self._render_claims_as_answer(
            [claim.claim for claim in corrected_claim_results]
        ) or self._correct_answer(claim_results)
        answer_confidence = self._confidence(claim_results)
        corrected_confidence = (
            self._confidence(corrected_claim_results)
            if corrected_claim_results
            else self._confidence(claim_results, verdict="grounded")
        )
    ...
    def run_detection(self, payload: DetectRequest) -> DetectResponse:
        raw_chunks, _evidence = self._retrieved_evidence(payload.question, payload.top_k)
        result = self._execute(
            mode="detect",
            question=payload.question,
